...
#define MGS2_CUTSCENE_FRAMESKIP_INLINE_HOOKS(X) \
    ...
    X(Act_GeorgeBonbori, "40 56 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 44 24 ?? 83 3D ?? ?? ?? ?? 00 48 8B F1 48 8B 81 ?? ?? ?? ?? 74 ?? 81 88 ?? ?? ?? ?? ?? ?? ?? ?? 48 8B 81 ?? ?? ?? ?? 81 88 ?? ?? ?? ?? ?? ?? ?? ?? E9 ?? ?? ?? ?? 81 A0 ?? ?? ?? ?? ?? ?? ?? ?? 48 8B 81 ?? ?? ?? ?? 48 89 9C 24 ?? ?? ?? ?? 48 89 AC 24 ?? ?? ?? ?? 48 89 BC 24", "MGS 2: Effect Speed Fix : user\\okajima\\effect\\george_bonbori.c -> Act()")
